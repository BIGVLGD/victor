...
router.post('/', requireAuth, (req, res) => {
  const { sku, name, category_id, dose, sell_price, cost_price, stock, threshold, supplier_cat_no } = req.body;
  if (!sku || !name) return res.status(400).json({ error: 'SKU and name required' });
  const info = db.prepare(`
    INSERT INTO products (sku, name, category_id, dose, sell_price, cost_price, stock, threshold, supplier_cat_no)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(sku, name, category_id || null, dose || '', sell_price || 0, cost_price || 0, stock || 0, threshold || 5, supplier_cat_no || '');
...
router.put('/:id', requireAuth, (req, res) => {
  const { sku, name, category_id, dose, sell_price, cost_price, stock, threshold, supplier_cat_no, active } = req.body;
  db.prepare(`
    UPDATE products SET sku=?, name=?, category_id=?, dose=?, sell_price=?, cost_price=?, stock=?, threshold=?, supplier_cat_no=?, active=?, updated_at=datetime('now')
    WHERE id=?
  `).run(sku, name, category_id || null, dose || '', sell_price || 0, cost_price || 0, stock || 0, threshold || 5, supplier_cat_no || '', active !== undefined ? active : 1, req.params.id);
